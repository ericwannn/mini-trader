...
from macro_collector.config import llm_api_key, llm_base_url, llm_model
from macro_collector.digest.generator import format_for_llm
from macro_collector.models import Article
from macro_collector.models.digest import generate_intro_prompt
...
_SYSTEM_PROMPT = (
    ...
    "4. 每条资讯使用 `### 文章: 标题`，并包含字段：来源、原文链接、内容摘要、"
    "涉及品种、方向判断、预测周期、分析逻辑（分步骤编号）\n"
    "5. 保留可点击的 Markdown 链接\n"
...
def generate_digest_via_llm(articles: list[Article], target_date: str) -> str:
    ...
    if not api_key:
        print(
            "❌ 未配置 LLM API Key。请设置环境变量 MACRO_LLM_API_KEY 或 OPENAI_API_KEY。\n"
            "   可选：MACRO_LLM_BASE_URL（OpenAI 兼容端点）、MACRO_LLM_MODEL（默认 gpt-4o-mini）"
        )
        sys.exit(1)
