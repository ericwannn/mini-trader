...
from minitrader.config import wechat_cookie, wechat_proxy
from minitrader.models import Article
from minitrader.utils import make_session, gentle_delay
...
class WeChatCollector(BaseCollector):
    ...
    def search(self, keyword: str, max_articles: int = 5) -> list[dict]:
        ...
        if "验证码" in r.text or "checkcode" in r.text.lower():
            ...
            print(
                "     可在项目根目录 .env 中配置 MINITRADER_WECHAT_COOKIE / MINITRADER_WECHAT_PROXY 后重试；"
                "或设置 MINITRADER_WECHAT_ENABLED=0 跳过微信源。详见 README。"
            )
            return []
