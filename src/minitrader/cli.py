"""minitrader CLI — collect / digest / limitup / frontend / all"""
...
from minitrader import __about__
...
from minitrader.db.sync import persist_articles, persist_digest, persist_topics
...
def do_digest(args: argparse.Namespace):
    ...
    if use_llm:
        from minitrader.digest.llm import generate_digest_via_llm
    ...
    print(f"\n完成。摘要文件: {path}")
    print(f"入库 digests({target_date}) → minitrader.db")
    if n_topics:
...
def do_limitup(args: argparse.Namespace):
    """采集涨停数据并入库"""
    from minitrader.limitup import collect_limitup_data
...
def do_frontend(args: argparse.Namespace):
    ...
    from minitrader.service import start_server
...
def do_serve(args: argparse.Namespace):
    ...
    from minitrader.service import restart_server, start_server, status_server, stop_server
...
def main(argv: list[str] | None = None):
    ...
    p_digest.add_argument(
        "--llm",
        action="store_true",
        help="使用 LLM 生成摘要（需 MINITRADER_LLM_API_KEY 或 OPENAI_API_KEY）",
    )
